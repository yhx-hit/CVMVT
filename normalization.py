...
class ComplexLayerNorm2d(_ComplexLayerNorm):
    ...
    def forward(self, input):
        ...
        if self.training or (not self.training and not self.track_running_stats):
            # calculate mean of real and imaginary part
            # mean does not support automatic differentiation for outputs with complex dtype.
            mean_r = input.real.mean([1, 2])
            mean_i = input.imag.mean([1, 2])
            mean = mean_r + 1j * mean_i
        else:
            mean = self.running_mean
        ...
        if self.training or (not self.training and not self.track_running_stats):
            ...
            Crr = 1. / n * input.real.pow(2).sum(dim=[1, 2]) + self.eps
            Cii = 1. / n * input.imag.pow(2).sum(dim=[1, 2]) + self.eps
            Cri = (input.real.mul(input.imag)).mean(dim=[1, 2])
        else:
            Crr = self.running_covar[:, 0] + self.eps
            Cii = self.running_covar[:, 1] + self.eps
            Cri = self.running_covar[:, 2]  # +self.eps
...
if __name__ == "__main__":
    ...
    x = torch.randn((32,3,224)) # [B,C,H,W]
    xi = torch.randn((32,3,224))
    b, c, h = x.shape
    ...
    complex_LN=ComplexLayerNorm2d(b, eps=1e-12, affine=False, track_running_stats=True)
    complex_yin = complex_LN(x,xi)
